...
class CustomPlayerMCTS(DataPlayer):
    ...
    **********************************************************************
    ...
    def uct_search(self, state):
        ...
        for i in range(budget):
            ...
            self.backup_nega_max(node, reward)
    ...
    def tree_policy(self, node):
        while not node.state.terminal_test():
            if not self.fully_expanded(node):
                ...
                node = self.best_child(node, exploration_constant)
        return node
...
class CustomPlayerADS(DataPlayer):
    def get_action(self, state):
        if len (state.actions()) == 1:
            # dbstate = DebugState.from_state(state)
            # print (dbstate)
        ...
        if state.ply_count < 2:
            ...
            action = self.iterative_deepening(state)
        # dbstate = DebugState.from_state(state)
        # print (dbstate)
        if action is None:
            print("Incorrect action")
    ...
    def iterative_deepening(self, state):
        ...
        for d in range(1, base_depth + 1):
            best_move = self.alpha_beta_search(state, d)
