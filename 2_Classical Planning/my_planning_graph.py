...
class ActionLayer(BaseActionLayer):
    ...
    def _interference(self, actionA, actionB):
        ...
        for preconditionA in self.parents[actionA]:
            if ~preconditionA in self.children[actionB]:
                return True
        for preconditionB in self.parents[actionB]:
            if ~preconditionB in self.children[actionA]:
                return True
        return False
        raise NotImplementedError
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for preconditionA in self.parents[actionA]:
            for preconditionB in self.parents[actionB]:
                if self.parent_layer.is_mutex(preconditionA, preconditionB) or self.parent_layer.is_mutex(preconditionB, preconditionA):
                    return True
        return False
        raise NotImplementedError
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for actionA in self.parents[literalA]:
            for actionB in self.parents[literalB]:
                if not self.parent_layer.is_mutex(actionA, actionB) or not self.parent_layer.is_mutex(actionB, actionA):
                    return False
        return True
        raise NotImplementedError
